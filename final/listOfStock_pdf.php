    ...
    class PDF extends FPDF
    {
        ...
        function FancyTable($header,$r)
        {
            ...
            while($row = mysql_fetch_array($r))
            {
                ...
                $this->Cell($w[0],6,$row['prod_id'],'LR',0,'R',$fill);
                $this->Cell($w[1],6,$row['prod_name'],'LR',0,'L',$fill);
                $this->Cell($w[2],6,$row['prod_desc'],'LR',0,'J',$fill);
                $this->Cell($w[3],6,'Php '.$price.'','LR',0,'R',$fill);
                $this->Cell($w[4],6,$row['quantity_on_hand'],'LR',0,'R',$fill);
                $this->Ln();
                $fill=!$fill;
            }
        ...
        $pdf=new PDF();
        $header=array('Code','Name','Description','Price','Qty');
        $pdf->AddPage();
        $date = date("F j, Y");
        $date1 = date("m_d_Y");
        $pdf->SetXY(20, 30);
        $pdf->Cell(0,20,'List of Stocks(Summary) as of '.$date.'',0,1,'C');
        $pdf->Cell(0,10,'',0,1);
        $pdf->FancyTable($header,$r);
        $pdf->Output('listOfStocks_'.$date1.'.pdf','D');
