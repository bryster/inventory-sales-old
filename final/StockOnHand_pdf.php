    ...
    class PDF extends FPDF
    {
        ...
        function FancyTable($header,$r)
        {
            ...
            while($row = mysql_fetch_array($r))
            {
                ...
                $this->Cell($w[2],6,$row['quantity_on_hand'],'LR',0,'J',$fill);
                $this->Ln();
                $fill=!$fill;
            }
        ...
        $pdf=new PDF();
        $header=array('Code','Name','Quantity on Hand');
        $pdf->AddPage();
        $date = date("F j, Y");
        $date1 = date("m_d_Y");
        $pdf->SetXY(20, 30);
        $pdf->Cell(0,20,'Stocks on Hand as of '.$date.'',0,1,'C');
        $pdf->Cell(0,10,'',0,1);
        $pdf->FancyTable($header,$r);
        $pdf->Output('StockOnHand'.$date1.'.pdf','D');
